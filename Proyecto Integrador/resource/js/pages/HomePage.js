...
document.addEventListener("submit", (e) => {
  ...
  console.log(
    `cadena_busqueda: ${location.search.split("&s=")[0].split("?q=")[1]}`
  );
});
...
const busqueda = (cadena_busqueda) => {
  let array_busqueda = articulos.filter(
    (objeto) =>
      objeto.nombre.includes(cadena_busqueda) ||
      objeto.nombre.includes(
        cadena_busqueda.charAt(0).toUpperCase() + cadena_busqueda.slice(1)
      )
  );
